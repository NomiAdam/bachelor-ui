...
describe('AutocompleteSelect', () => {
    ...
    it('should match the snapshot with options', () => {
        expect(shallow(<Select visible={true} options={mockSuggestions} onClick={mockClick}/>)).toMatchSnapshot();
    });
    ...
    it('should have matching length when visible of Option components', () => {
        const wrapper = shallow(<Select visible={true} options={mockSuggestions} onClick={mockClick} />);
        expect( wrapper.find(Option).length).toBe(mockSuggestions.length);
    });
    it('should have length of zero when not visible', () => {
        const wrapper = shallow(<Select visible={false} options={mockSuggestions} />);
    ...
    it('should have border set to none when suggestions are empty Array', () => {
        expect(shallow(<Select options={[]} />)).toHaveStyleRule('border','none');
    });
});
